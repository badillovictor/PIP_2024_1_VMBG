...
class MyApp(QtWidgets.QMainWindow, grafica.Ui_MainWindow):
    def __init__(self):
        ...
        self.movimiento_enemigo_timer.setInterval(1000)
        ...
        for i in range(1, len(self.personajes)):
            if self.personajes[0] == self.personajes[i]:
                ...
                if len(self.personajes) == 1:
                    QtWidgets.QMessageBox.information(self, "Juego Terminado", "¡Has derrotado a todos los enemigos!")
                    self.btn_action.setText("INICIAR")
                    self.deshabilitar_botones()
    ...
    def actualizar_puntaje(self):
        self.puntaje += 1
        self.puntuacion.setText(str(self.puntaje))
